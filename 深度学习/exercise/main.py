...
#读取csv数据
train_data = pandas.read_csv("./data/kaggle_house_pred_train.csv")
test_data = pandas.read_csv("./data/kaggle_house_pred_test.csv")
...
#在这里修改了一些东西,参考https://blog.csdn.net/qq_39385231/article/details/142305252,在这里进行强制转换之后有一步tensor设置float32的操作，因为pytorch网络训练参数默认是float32
#把数据分成训练数据和测试数
...
class MyNet(torch.nn.Module):
    def __init__(self, in_put, hidden, hidden1,hidden2,hidden3, out_put):
        super().__init__()
        self.linear1 = torch.nn.Linear(in_put, hidden)
        self.linear2 = torch.nn.Linear(hidden, hidden1)
        self.linear3 = torch.nn.Linear(hidden1, hidden2)
        self.linear4 = torch.nn.Linear(hidden2, hidden3)
        self.linear5 = torch.nn.Linear(hidden3, out_put)
...
hidden, hidden1 ,hidden2,hidden3,out_put = 256, 128, 256,128,1
model = MyNet(in_features, hidden, hidden1, hidden2,hidden3,out_put).to(device)
